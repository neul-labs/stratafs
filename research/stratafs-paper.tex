...
\title{StrataFS: Towards Semantic, Searchable, and Agentic Filesystems for the AI Era}
...
We present StrataFS, a semantic filesystem designed to address these challenges through several key contributions:
...
The Model Context Protocol (MCP) \cite{anthropic2024mcp} standardizes how AI applications connect to data sources, enabling agents to dynamically discover and query available context. StrataFS implements MCP to serve as a universal context provider for any MCP-compatible agent.
...
Recent work on enterprise RAG \cite{gao2023retrieval} highlights the importance of connecting LLMs to organizational knowledge, yet most solutions require uploading sensitive data to cloud services. StrataFS addresses this gap with a privacy-preserving, local-first architecture.

\section{Architecture}

StrataFS employs a layered architecture that separates concerns while enabling rich interactions between components. Figure~\ref{fig:architecture} illustrates the overall system design.
...
\end{tikzpicture}
\caption{StrataFS system architecture showing the flow from storage sources through processing pipeline to search and API layers.}
...
Extracted text undergoes chunking to create segments appropriate for embedding models. StrataFS implements multiple chunking strategies:
...
StrataFS stores all metadata and embeddings in SQLite databases with two key extensions:
...
StrataFS exposes functionality through multiple interfaces:
...
StrataFS is implemented in Go (approximately 15,000 lines) for cross-platform static binaries and excellent concurrency.
...
We evaluate StrataFS across indexing performance, search quality, and resource consumption with comprehensive benchmarks.
...
StrataFS (KW) & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} \\
StrataFS (Sem) & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} \\
StrataFS (Hyb) & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} \\
\bottomrule
\end{tabular}
\label{tab:quality}
\end{center}
...
\toprule
\textbf{Feature} & \textbf{StrataFS} & \textbf{Elasticsearch} & \textbf{Sourcegraph} & \textbf{GitHub Search} & \textbf{ripgrep} & \textbf{LlamaIndex} \\
\midrule
...
Table~\ref{tab:features} compares features. StrataFS uniquely combines semantic search, privacy, OS integration, and agent support without requiring GPU or cloud services.
...
\midrule
StrataFS & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} \\
...
StrataFS trades indexing time for semantic capabilities. Query latency is competitive with Elasticsearch while requiring no separate service.
...
StrataFS processes all data locally using ONNX runtime. No network calls occur during indexing or search, making it suitable for air-gapped environments and sensitive data.

\section{Discussion}

\subsection{The Filesystem as Knowledge Infrastructure}

Our experience building and using StrataFS suggests that the filesystem abstraction requires fundamental evolution. The traditional model of files as opaque byte sequences organized by path fails to capture content semantics, relationships, or context.
...
StrataFS's MCP implementation provides these through a standardized protocol, enabling any MCP-compatible agent to leverage the semantic index.
...
We presented StrataFS, a semantic filesystem that transforms passive storage into intelligent knowledge infrastructure. By combining full-text and vector search, StrataFS enables both precise queries and semantic understanding. The privacy-first architecture makes semantic search viable for sensitive data while universal connectors aggregate knowledge from diverse sources.
...
As AI agents become central to knowledge work, filesystems must evolve from storage layers to knowledge infrastructure. StrataFS represents a step toward this future---one where finding information is as natural as asking a question.

\section*{Data Availability}

Benchmark scripts, query sets, and evaluation data are available at \url{https://github.com/dipankar/stratafs/research/benchmarks}. The Linux Kernel dataset is publicly available. Synthetic enterprise data generation scripts are included.
