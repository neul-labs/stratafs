#!/bin/bash
# AgentFS Linux AppImage Builder
...
APPDIR="$BUILD_DIR/AgentFS.AppDir"

print_info "Building AgentFS AppImage v$VERSION for $ARCH"
...
setup_appdir() {
    ...
    mkdir -p "$APPDIR/usr/share/agentfs"
    mkdir -p "$APPDIR/usr/share/doc/agentfs"
}
...
copy_files() {
    ...
    # Copy main binary
    cp "$BINARY_PATH" "$APPDIR/usr/bin/agentfs"
    chmod +x "$APPDIR/usr/bin/agentfs"
    ...
    if [ -f "$PROJECT_ROOT/README.md" ]; then
        cp "$PROJECT_ROOT/README.md" "$APPDIR/usr/share/doc/agentfs/"
    fi

    if [ -f "$PROJECT_ROOT/LICENSE" ]; then
        cp "$PROJECT_ROOT/LICENSE" "$APPDIR/usr/share/doc/agentfs/"
    fi

    # Create default configuration
    cat > "$APPDIR/usr/share/agentfs/default-config.json" << 'EOF'
{
  "version": "0.2.0",
  "agent_dir": ".agentfs",
  "global_dir": "~/.agentfs",
...
create_desktop_entry() {
    ...
    cat > "$APPDIR/agentfs.desktop" << EOF
[Desktop Entry]
Type=Application
Name=AgentFS
...
EOF
    ...
    cp "$APPDIR/agentfs.desktop" "$APPDIR/usr/share/applications/"
}
...
# Initialize AgentFS config if it doesn't exist
if [ ! -d "$HOME/.agentfs" ]; then
    echo "Initializing AgentFS configuration..."
    mkdir -p "$HOME/.agentfs"
    if [ -f "$HERE/usr/share/agentfs/default-config.json" ]; then
        cp "$HERE/usr/share/agentfs/default-config.json" "$HOME/.agentfs/config.json"
    fi
fi
...
case "$1" in
    --config)
        shift
        exec "$HERE/usr/bin/agentfs" config "$@"
        ;;
    --source)
        shift
        exec "$HERE/usr/bin/agentfs" source "$@"
        ;;
    --help|help)
        exec "$HERE/usr/bin/agentfs" --help
        ;;
    --version|version)
        exec "$HERE/usr/bin/agentfs" --version
        ;;
    --desktop)
        # Desktop mode: run in background with GUI integration
        if ! pgrep -f "agentfs" > /dev/null; then
            nohup "$HERE/usr/bin/agentfs" > "$HOME/.agentfs/desktop.log" 2>&1 &
            echo "AgentFS started in background. Check status at http://localhost:8080/health"
        else
            echo "AgentFS is already running"
        fi
        ;;
    --stop)
        # Stop AgentFS
        pkill -f "agentfs" && echo "AgentFS stopped" || echo "AgentFS not running"
        ;;
    *)
        # Default: run AgentFS with all arguments
        exec "$HERE/usr/bin/agentfs" "$@"
        ;;
esac
EOF
...
create_icon() {
    ...
    if [ -f "$SCRIPT_DIR/assets/agentfs.png" ]; then
        cp "$SCRIPT_DIR/assets/agentfs.png" "$APPDIR/agentfs.png"
        cp "$SCRIPT_DIR/assets/agentfs.png" "$APPDIR/usr/share/icons/hicolor/256x256/apps/agentfs.png"
    else
        print_warning "Icon not found, creating placeholder"
        # Create a simple colored square as placeholder
        if command -v convert >/dev/null 2>&1; then
            convert -size 256x256 xc:'#007AFF' "$APPDIR/agentfs.png"
            cp "$APPDIR/agentfs.png" "$APPDIR/usr/share/icons/hicolor/256x256/apps/agentfs.png"
        else
            print_warning "ImageMagick not found, using text icon"
            echo "AgentFS" > "$APPDIR/agentfs.png"
            cp "$APPDIR/agentfs.png" "$APPDIR/usr/share/icons/hicolor/256x256/apps/agentfs.png"
        fi
    fi
}
...
create_launcher() {
    ...
    cat > "$APPDIR/usr/bin/agentfs-gui" << 'EOF'
#!/bin/bash
...
# Run in background
nohup agentfs > "$HOME/.agentfs/desktop.log" 2>&1 &
...
EOF
    ...
    chmod +x "$APPDIR/usr/bin/agentfs-gui"
}
...
build_appimage() {
    ...
    "$SCRIPT_DIR/appimagetool-$ARCH.AppImage" \
        ...
        AgentFS.AppDir \
        "$OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage"
    ...
    chmod +x "$OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage"
    print_success "AppImage built: $OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage"
}
...
generate_checksum() {
    ...
    local appimage_path="$OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage"
...
#!/bin/bash
# AgentFS Desktop Integration Installer

APPIMAGE_PATH="\$1"
if [ -z "\$APPIMAGE_PATH" ]; then
    APPIMAGE_PATH="./AgentFS-$VERSION-$ARCH.AppImage"
fi
...
echo "Installing AgentFS desktop integration..."

# Create desktop entry
mkdir -p "\$HOME/.local/share/applications"
cat > "\$HOME/.local/share/applications/agentfs.desktop" << DESKTOP_EOF
[Desktop Entry]
Type=Application
Name=AgentFS
Comment=The Agentic Filesystem for AI agents
...
"\$APPIMAGE_PATH" --appimage-extract usr/share/icons/hicolor/256x256/apps/agentfs.png >/dev/null 2>&1
if [ -f "squashfs-root/usr/share/icons/hicolor/256x256/apps/agentfs.png" ]; then
    mkdir -p "\$HOME/.local/share/icons/hicolor/256x256/apps"
    cp "squashfs-root/usr/share/icons/hicolor/256x256/apps/agentfs.png" "\$HOME/.local/share/icons/hicolor/256x256/apps/"
    rm -rf squashfs-root
fi
...
echo "You can now find AgentFS in your application menu."
EOF
...
main() {
    ...
    print_info "AppImage: $OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage"
    print_info "Size: $(du -h "$OUTPUT_DIR/AgentFS-$VERSION-$ARCH.AppImage" | cut -f1)"

    echo ""
    print_info "Usage instructions:"
    print_info "1. Make executable: chmod +x AgentFS-$VERSION-$ARCH.AppImage"
    print_info "2. Run directly: ./AgentFS-$VERSION-$ARCH.AppImage"
    print_info "3. Desktop mode: ./AgentFS-$VERSION-$ARCH.AppImage --desktop"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --arch)
            ...
            ;;
        --help)
            echo "AgentFS AppImage Build Script"
            echo ""
