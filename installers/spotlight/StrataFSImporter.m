/*
 * StrataFS Spotlight Importer
 *
 * This Spotlight importer plugin indexes StrataFS metadata into macOS Spotlight.
 * It reads the StrataFS database and exposes semantic metadata for search.
 *
 * Build: xcodebuild -project StrataFSImporter.xcodeproj
...
Boolean GetMetadataForFile(void *thisInterface,
    ...
    @autoreleasepool {
        ...
        NSString *dbPath = [homeDir stringByAppendingPathComponent:@".stratafs/stratafs.db"];

        // Check if file exists in StrataFS
        sqlite3 *db;
        ...
        // Custom StrataFS attributes
        if (checksum) {
            attrs[@"org_stratafs_checksum"] = [NSString stringWithUTF8String:checksum];
        }

        attrs[@"org_stratafs_file_id"] = @(fileId);
        attrs[@"org_stratafs_indexed"] = @YES;
