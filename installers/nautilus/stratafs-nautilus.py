...
Adds context menu items for StrataFS operations in GNOME Files.
...
class StrataFSExtension(GObject.GObject, Nautilus.MenuProvider):
    """Nautilus extension for StrataFS file operations."""

    def __init__(self):
        super().__init__()
        self.api_url = os.environ.get('STRATAFS_API_URL', 'http://localhost:8080')
    ...
    def get_file_items(self, files):
        ...
        if len(files) == 1:
            ...
            if file_obj.get_uri_scheme() == 'file':
                ...
                item = Nautilus.MenuItem(
                    name='StrataFS::ViewMetadata',
                    label='View StrataFS Metadata',
                ...
                # View Chunks action
                item = Nautilus.MenuItem(
                    name='StrataFS::ViewChunks',
                    label='View StrataFS Chunks',
        ...
        if filepaths:
            # Reindex action
            item = Nautilus.MenuItem(
                name='StrataFS::Reindex',
                label=f'Reindex in StrataFS ({len(filepaths)} file{"s" if len(filepaths) > 1 else ""})',
    ...
    def get_background_items(self, current_folder):
        ...
        # Add source action
        item = Nautilus.MenuItem(
            name='StrataFS::AddSource',
            label='Add Folder to StrataFS',
            tip='Add this folder as an StrataFS source',
        ...
        item = Nautilus.MenuItem(
            name='StrataFS::ExportMetadata',
            label='Export StrataFS Metadata Here',
    ...
    def _on_view_metadata(self, menu, filepath):
        ...
        try:
            result = subprocess.run(
                ['stratafs', 'file', 'info', filepath],
                capture_output=True,
                text=True,
                timeout=10
            )
            self._show_dialog('StrataFS Metadata', result.stdout or result.stderr or 'No metadata found')
        except subprocess.TimeoutExpired:
            self._show_dialog('Error', 'Request timed out')
        except FileNotFoundError:
            self._show_dialog('Error', 'stratafs command not found. Is StrataFS installed?')
    ...
    def _on_view_chunks(self, menu, filepath):
        ...
        try:
            result = subprocess.run(
                ['stratafs', 'file', 'chunks', filepath],
                capture_output=True,
                text=True,
                timeout=10
            )
            self._show_dialog('StrataFS Chunks', result.stdout or result.stderr or 'No chunks found')
        except subprocess.TimeoutExpired:
            self._show_dialog('Error', 'Request timed out')
        except FileNotFoundError:
            self._show_dialog('Error', 'stratafs command not found. Is StrataFS installed?')
    ...
    def _on_search_similar(self, menu, filepath):
        ...
        try:
            ...
            result = subprocess.run(
                ['stratafs', 'file', 'chunks', filepath, '--limit', '1', '--format', 'text'],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def _on_reindex(self, menu, filepaths):
        ...
        try:
            for filepath in filepaths:
                subprocess.run(
                    ['stratafs', 'file', 'reindex', filepath],
                    capture_output=True,
                    timeout=10
                )
    ...
    def _on_add_source(self, menu, folderpath):
        """Add folder as an StrataFS source."""
        try:
            result = subprocess.run(
                ['stratafs', 'source', 'add', folderpath],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def _on_export_metadata(self, menu, folderpath):
        ...
        try:
            result = subprocess.run(
                ['stratafs', 'fs', 'export', '--output', folderpath],
                capture_output=True,
                text=True,
                timeout=60
            )
