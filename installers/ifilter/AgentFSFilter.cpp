/*
 * AgentFS Windows Search IFilter
 *
 * This IFilter implementation exposes AgentFS indexed content to Windows Search.
 * It reads chunk data from the AgentFS database and provides it to the indexer.
 *
 * Build with Visual Studio or cl.exe
 * Register with: regsvr32 AgentFSFilter.dll
 */
...
static const GUID CLSID_AgentFSFilter =
{ 0xa1b2c3d4, 0xe5f6, 0x7890, { 0xab, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x90 } };

class AgentFSFilter : public IFilter, public IPersistFile, public IPersistStream {
private:
    LONG m_refCount;
...
public:
    AgentFSFilter() : m_refCount(1), m_currentChunk(0), m_currentPos(0), m_initialized(false) {}
    virtual ~AgentFSFilter() {}
...
// Class factory
class AgentFSFilterFactory : public IClassFactory {
private:
    LONG m_refCount;

public:
    AgentFSFilterFactory() : m_refCount(1) {}
    ...
    STDMETHODIMP CreateInstance(IUnknown *pUnkOuter, REFIID riid, void **ppv) {
        ...
        AgentFSFilter *filter = new AgentFSFilter();
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv) {
    if (rclsid == CLSID_AgentFSFilter) {
        AgentFSFilterFactory *factory = new AgentFSFilterFactory();
