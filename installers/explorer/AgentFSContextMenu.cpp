/*
 * AgentFS Windows Explorer Context Menu Extension
 *
...
static const GUID CLSID_AgentFSContextMenu =
{ 0xb2c3d4e5, 0xf6a7, 0x8901, { 0xbc, 0xde, 0xf2, 0x34, 0x56, 0x78, 0x90, 0x12 } };

class AgentFSContextMenu : public IShellExtInit, public IContextMenu {
private:
    LONG m_refCount;
...
public:
    AgentFSContextMenu() : m_refCount(1), m_isDirectory(false) {}
    virtual ~AgentFSContextMenu() {}
    ...
    STDMETHODIMP QueryContextMenu(HMENU hmenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags) {
        ...
        if (m_isDirectory) {
            ...
            InsertMenuW(hSubmenu, 0, MF_BYPOSITION | MF_STRING, idCmdFirst + 0, L"Add to AgentFS");
            InsertMenuW(hSubmenu, 1, MF_BYPOSITION | MF_STRING, idCmdFirst + 1, L"Export Metadata Here");
        } else {
            // File menu items
            InsertMenuW(hSubmenu, 0, MF_BYPOSITION | MF_STRING, idCmdFirst + 0, L"View AgentFS Metadata");
        ...
        mii.dwTypeData = const_cast<LPWSTR>(L"AgentFS");
...
private:
    HRESULT RunAgentFS(const std::vector<std::wstring>& args, bool showOutput = true) {
        std::wstring cmdLine = L"\"C:\\Program Files\\AgentFS\\agentfs.exe\"";
    ...
    HRESULT ViewMetadata() {
        ...
        return RunAgentFS({ L"file", L"info", m_selectedFiles[0] });
    }

    HRESULT ViewChunks() {
        if (m_selectedFiles.empty()) return E_FAIL;
        return RunAgentFS({ L"file", L"chunks", m_selectedFiles[0] });
    }
    ...
    HRESULT Reindex() {
        for (const auto& file : m_selectedFiles) {
            RunAgentFS({ L"file", L"reindex", file }, false);
        }

        std::wstring msg = L"Queued " + std::to_wstring(m_selectedFiles.size()) + L" file(s) for reindexing";
        MessageBoxW(nullptr, msg.c_str(), L"AgentFS", MB_OK | MB_ICONINFORMATION);
        return S_OK;
    }

    HRESULT AddSource() {
        if (m_selectedFiles.empty()) return E_FAIL;
        return RunAgentFS({ L"source", L"add", L"--path", m_selectedFiles[0] });
    }

    HRESULT ExportMetadata() {
        if (m_selectedFiles.empty()) return E_FAIL;
        return RunAgentFS({ L"fs", L"export", L"--output", m_selectedFiles[0] });
    }
};

// Class factory
class AgentFSContextMenuFactory : public IClassFactory {
private:
    LONG m_refCount;

public:
    AgentFSContextMenuFactory() : m_refCount(1) {}
    ...
    STDMETHODIMP CreateInstance(IUnknown *pUnkOuter, REFIID riid, void **ppv) {
        ...
        AgentFSContextMenu *menu = new AgentFSContextMenu();
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv) {
    if (rclsid == CLSID_AgentFSContextMenu) {
        AgentFSContextMenuFactory *factory = new AgentFSContextMenuFactory();
