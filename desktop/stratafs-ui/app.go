...
func NewApp() *App {
	...
	return &App{
		...
		configDir: filepath.Join(homeDir, ".stratafs"),
	}
}
...
// GetStatus returns the current status of StrataFS
...
// isProcessRunning checks if stratafs daemon process is running
...
// StartStrataFS starts the StrataFS daemon
func (a *App) StartStrataFS() error {
	if a.isProcessRunning() {
		return nil // Already running, not an error
	}

	// Find stratafs binary
	stratafsPath := a.findStrataFSBinary()
	if stratafsPath == "" {
		return fmt.Errorf("stratafs binary not found - ensure it's bundled with the app or in PATH")
	}

	// Start the daemon in background
	cmd := exec.Command(stratafsPath, "serve", "--config-dir", a.configDir)
	cmd.Stdout = nil
	cmd.Stderr = nil

	if err := cmd.Start(); err != nil {
		return fmt.Errorf("failed to start StrataFS: %w", err)
	}
	...
	return fmt.Errorf("StrataFS started but API not responding")
}

// StopStrataFS stops the StrataFS daemon
func (a *App) StopStrataFS() error {
	if !a.isProcessRunning() {
	...
	if runtime.GOOS == "windows" {
		cmd = exec.Command("taskkill", "/F", "/IM", "stratafs.exe")
	} else {
		// Use pkill with exact match to avoid killing stratafs-ui
		cmd = exec.Command("pkill", "-x", "stratafs")
	}
...
// RestartStrataFS restarts the StrataFS daemon
func (a *App) RestartStrataFS() error {
	if a.isProcessRunning() {
		if err := a.StopStrataFS(); err != nil {
			return err
		}
		time.Sleep(2 * time.Second)
	}
	return a.StartStrataFS()
}
...
func (a *App) GetConfig() (*Config, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("config not found - please initialize StrataFS first")
		}
...
func (a *App) InitConfig() error {
	cmd := exec.Command("stratafs", "config", "init", "--config-dir", a.configDir)
...
func (a *App) ExportSource(sourceName, outputPath string) error {
	cmd := exec.Command("stratafs", "fs", "export",
		"--source", sourceName,
		"--output", outputPath,
...
// GetMountStatus checks if StrataFS is mounted
...
// MountFilesystem mounts StrataFS as a FUSE filesystem
func (a *App) MountFilesystem() error {
	...
	// Find stratafs binary (same logic as StartStrataFS)
	stratafsPath := a.findStrataFSBinary()
	if stratafsPath == "" {
		return fmt.Errorf("stratafs binary not found")
	}

	// Start the mount command
	cmd := exec.Command(stratafsPath, "mount", "--mount-point", mountPoint, "--config-dir", a.configDir)
	output, err := cmd.CombinedOutput()
	if err != nil {
...
// UnmountFilesystem unmounts the StrataFS FUSE filesystem
...
// findStrataFSBinary locates the stratafs binary
func (a *App) findStrataFSBinary() string {
	// Get the directory of the current executable
	if execPath, err := os.Executable(); err == nil {
		execDir := filepath.Dir(execPath)
		bundledPaths := []string{
			filepath.Join(execDir, "stratafs"),
			filepath.Join(execDir, "bin", "stratafs"),
			filepath.Join(execDir, "..", "Resources", "stratafs"),
		}
		if runtime.GOOS == "windows" {
			bundledPaths = []string{
				filepath.Join(execDir, "stratafs.exe"),
				filepath.Join(execDir, "bin", "stratafs.exe"),
			}
		}
	...
	// Check PATH
	if path, err := exec.LookPath("stratafs"); err == nil {
		return path
	}
	...
	if runtime.GOOS == "windows" {
		fallbackPaths = []string{
			filepath.Join(homeDir, "AppData", "Local", "StrataFS", "stratafs.exe"),
			"C:\\Program Files\\StrataFS\\stratafs.exe",
		}
	} else {
		fallbackPaths = []string{
			"/usr/local/bin/stratafs",
			"/usr/bin/stratafs",
			filepath.Join(homeDir, ".local/bin/stratafs"),
		}
	}
