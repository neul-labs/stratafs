...
function getBinaryDir() {
  const dir = path.join(os.homedir(), ".stratafs", "bin");
...
async function downloadBinary() {
  ...
  const archiveName = `stratafs-v${VERSION}-${platform}-${arch}`;
  ...
  console.log(`Downloading StrataFS ${VERSION} for ${platform}/${arch}...`);

  await downloadFile(url, archivePath);

  if (archiveExt === "zip") {
    execSync(`unzip -o "${archivePath}" -d "${binaryDir}"`, { stdio: "inherit" });
    const extractedBinary = path.join(binaryDir, `${platform}-${arch}`, "stratafs.exe");
    if (fs.existsSync(extractedBinary)) {
      fs.renameSync(extractedBinary, binaryPath);
    }
  } else {
    execSync(`tar -xzf "${archivePath}" -C "${binaryDir}"`, { stdio: "inherit" });
    const extractedBinary = path.join(binaryDir, `${platform}-${arch}`, "stratafs");
    if (fs.existsSync(extractedBinary)) {
      fs.renameSync(extractedBinary, binaryPath);
    }
  }
...
async function main() {
  try {
    ...
    console.log("StrataFS binary installed successfully.");
  } catch (err) {
    console.error("Failed to install StrataFS binary:", err.message);
    process.exit(0);
  }
}
