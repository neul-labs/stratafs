"""Download and cache the AgentFS native binary."""
...
GITHUB_RELEASES_URL = "https://github.com/dipankarsarkar/agentfs/releases/download"
...
def get_binary_dir():
    ...
    cache_dir = os.path.expanduser("~/.agentfs/bin")
    os.makedirs(cache_dir, exist_ok=True)
    return cache_dir


def get_binary_path():
    """Return the path to the cached AgentFS binary."""
    binary_name = "agentfs.exe" if get_platform() == "windows" else "agentfs"
    return os.path.join(get_binary_dir(), binary_name)


def download_binary(version=None):
    """Download the AgentFS binary for the current platform."""
    if version is None:
        version = os.environ.get("AGENTFS_VERSION", DEFAULT_VERSION)
    ...
    archive_name = f"agentfs-v{version}-{plat}-{arch}"
    if plat == "windows":
        archive_ext = "zip"
    else:
        archive_ext = "tar.gz"
    ...
    print(f"Downloading AgentFS {version} for {plat}/{arch}...")
    print(f"URL: {url}")
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to download AgentFS binary from {url}: {exc}"
        ) from exc

    if archive_ext == "zip":
        with zipfile.ZipFile(archive_path, "r") as zf:
            for member in zf.namelist():
                if member.endswith("agentfs.exe"):
        ...
        with tarfile.open(archive_path, "r:gz") as tf:
            for member in tf.getmembers():
                if member.name.endswith("agentfs"):
